...
def _providers() -> List[Provider]:
    ...
    Kept deliberately narrow. qwen/qwen3.8-27b and openai/gpt-oss-safeguard-20b
    were added for extra token headroom and then removed: qwen read "7 PM to
    9 PM" as [19,20,21], over-applying the sub-hour rounding rule to a whole-
    hour window, and safeguard missed "bar discharging" as a prohibition. A
    weak link is reached exactly when the strong ones are throttled, so it
    trades 25 points of interpretation for 3 points of latency. Any model
    added here must first be checked on plain whole-hour window arithmetic,
    not only on unusual phrasings.
